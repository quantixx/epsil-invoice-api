...
class EventGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Event entity")
        ...
        .repeat(2) {
            ...
            .get("/invoiceapi/api/events")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new event")
            .post("/invoiceapi/api/events")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "slug":"SAMPLE_TEXT", "name":"SAMPLE_TEXT", "virtual":null, "startsOn":"2020-01-01T00:00:00.000Z", "endsOn":"2020-01-01T00:00:00.000Z"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_event_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created event")
                .get("/invoiceapi${new_event_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created event")
            .delete("/invoiceapi${new_event_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
