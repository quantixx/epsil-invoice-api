...
class CountryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Country entity")
        ...
        .repeat(2) {
            ...
            .get("/invoiceapi/api/countries")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new country")
            .post("/invoiceapi/api/countries")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "iso2":"SAMPLE_TEXT", "iso3":"SAMPLE_TEXT", "m49":"0", "name":"SAMPLE_TEXT", "officialNameEn":"SAMPLE_TEXT", "officialNameFr":"SAMPLE_TEXT", "dial":"SAMPLE_TEXT", "continent":null, "tld":"SAMPLE_TEXT", "flag32":"SAMPLE_TEXT", "flag128":"SAMPLE_TEXT", "latitude":"SAMPLE_TEXT", "longitude":"SAMPLE_TEXT", "zoom":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_country_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created country")
                .get("/invoiceapi${new_country_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created country")
            .delete("/invoiceapi${new_country_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
