...
class SequenceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Sequence entity")
        ...
        .repeat(2) {
            ...
            .get("/invoiceapi/api/sequences")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new sequence")
            .post("/invoiceapi/api/sequences")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "docType":null, "next":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_sequence_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created sequence")
                .get("/invoiceapi${new_sequence_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created sequence")
            .delete("/invoiceapi${new_sequence_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
