...
class AddressGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Address entity")
        ...
        .repeat(2) {
            ...
            .get("/invoiceapi/api/addresses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new address")
            .post("/invoiceapi/api/addresses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "street":"SAMPLE_TEXT", "street2":"SAMPLE_TEXT", "city":"SAMPLE_TEXT", "state":"SAMPLE_TEXT", "zipCode":"SAMPLE_TEXT", "additionalInformation":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_address_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created address")
                .get("/invoiceapi${new_address_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created address")
            .delete("/invoiceapi${new_address_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
