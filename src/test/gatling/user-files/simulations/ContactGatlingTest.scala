...
class ContactGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Contact entity")
        ...
        .repeat(2) {
            ...
            .get("/invoiceapi/api/contacts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new contact")
            .post("/invoiceapi/api/contacts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "firstName":"SAMPLE_TEXT", "lastName":"SAMPLE_TEXT", "activated":null, "email":"SAMPLE_TEXT", "phoneArea":"SAMPLE_TEXT", "phoneNumber":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_contact_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created contact")
                .get("/invoiceapi${new_contact_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created contact")
            .delete("/invoiceapi${new_contact_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
